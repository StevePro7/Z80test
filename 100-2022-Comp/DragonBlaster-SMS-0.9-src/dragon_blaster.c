...
#define FOR_EACH_ENEMY(enm) \
	enm = enemies;          \
	for (int i = ENEMY_MAX; i; i--, enm++)
#define FOR_EACH_ENEMY_SHOT(sht) \
	sht = enemy_shots;           \
	for (int i = ENEMY_SHOT_MAX; i; i--, sht++)
...
void handle_player_input()
{
	...
	{
		if (player.x > PLAYER_LEFT)
			player.x -= PLAYER_SPEED;
	}
	else if (joy & PORT_A_KEY_RIGHT)
	{
		if (player.x < PLAYER_RIGHT)
			player.x += PLAYER_SPEED;
	}
...
char powerup_base_tile(char type)
{
	...
	{
	case POWERUP_LIGHTINING:
		return POWERUP_LIGHTINING_TILE;
	case POWERUP_FIRE:
		return POWERUP_FIRE_TILE;
	case POWERUP_WIND:
		return POWERUP_WIND_TILE;
	}
...
void handle_icons()
{
	...
	if (!ply_ctl.powerup1_active)
		tile += 4;
	icons[0].base_tile = tile;

	tile = powerup_base_tile(ply_ctl.powerup2);
	if (ply_ctl.powerup2 && !ply_ctl.powerup2_active)
		tile += 4;
...
void handle_enemy_shots()
{
	...
	FOR_EACH_ENEMY_SHOT(sht)
	{
		...
		{
			move_actor(sht);
			if (sht->y < 0 || sht->y > (SCREEN_H - 16))
				sht->active = 0;
			if (sht->state == 1 && !sht->state_timer)
				sht->active = 0;
		}
	}
}
